...
function buildHtml(orders: Order[]): string {
  ...
  const rows = orders
    .map((o) => {
      ...
      return `
        <tr style="border-bottom:1px solid #e4e2d2">
          ...
          <td style="padding:10px 8px;vertical-align:top">${items}
            ${o.order_notes ? `<br><em style="color:#8a3b1a">Note: ${o.order_notes}</em>` : ""}
          </td>
  ...
  return `
  <div style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;color:#2b3419;max-width:760px">
    ...
    <table style="border-collapse:collapse;width:100%;font-size:14px">
      <thead>
        <tr style="background:#f0efe4;text-align:left">
