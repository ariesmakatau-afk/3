...
function buildHtml(orders: Order[]): string {
  ...
  const rows = orders
    .map((o) => {
      ...
      return `
        <tr style="border-bottom:1px solid #eadfdd">
          ...
          <td style="padding:10px 8px;vertical-align:top">${items}
            ${o.order_notes ? `<br><em style="color:#8a5f0f">Note: ${o.order_notes}</em>` : ""}
          </td>
  ...
  return `
  <div style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;color:#4a121c;max-width:760px">
    ...
    <table style="border-collapse:collapse;width:100%;font-size:14px">
      <thead>
        <tr style="background:#f6efee;text-align:left">
